...
def test_open_own_dataset(driver):
    ...
    uploaded_by_element = driver.find_element(By.XPATH, "//span[contains(text(), 'Uploaded by')]/../following-sibling::div/a")
    uploaded_by_name = uploaded_by_element.text.strip()
    
    assert logged_in_name == uploaded_by_name, f"Dataset verification failed: Expected owner '{logged_in_name}', but found '{uploaded_by_name}'"
    delayTimes(3)
...
def test_post_comment(driver):
    ...
    try:
        ...
        comment_item = driver.find_element(By.XPATH, f"//div[contains(@class, 'comment-item') and .//div[contains(@class, 'comment-text') and text()='{comment_content}']]")
    except NoSuchElementException:
    ...
    if normalized_logged_in != normalized_author:
         raise AssertionError(f"ERROR: No coincide el nombre del usuario con el autor. Expected: '{logged_in_name}', Found: '{author_name}'")
    ...
    if actual_content != comment_content:
        raise AssertionError(f"ERROR: El contenido del comentario es diferente. Expected: '{comment_content}', Found: '{actual_content}'")
...
def test_delete_comment(driver):
    ...
    try:
        WebDriverWait(driver, 5).until(
            expected_conditions.invisibility_of_element_located((By.CSS_SELECTOR, f"#comment-item-{comment_id}")) # This ID usually doesn't exist on the main container div based on previous code review, check selector
            # Reviewing view_dataset.html, the comment item is: <div class="comment-item" data-comment-id="${comment.id}">
            # It does not have an ID like #comment-item-{id}. It relies on data attribute.
            # But the user's previous code tried: expected_conditions.invisibility_of_element_located((By.CSS_SELECTOR, f"#comment-item-{comment_id}"))
            # Let's fix the selector to be robust: //div[@data-comment-id='{comment_id}']
        )
        WebDriverWait(driver, 5).until(
